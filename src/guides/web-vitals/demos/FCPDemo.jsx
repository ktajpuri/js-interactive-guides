...
export default function FCPDemo() {
  ...
  const simulate = async () => {
    ...
    bars.push({ id: 'ttfb', label: 'TTFB', value: ttfbDuration, color: 'bg-blue-500' });
    setWaterfall([...bars]);

    for (const r of RESOURCES) {
      if (r.delta > 0 && enabled[r.id]) {
        await sleep(r.delta);
        bars.push({ id: r.id, label: r.label, value: r.delta, color: 'bg-red-500' });
        setWaterfall([...bars]);
      }
    }
    ...
    if (fontBoost > 0) {
      bars.push({ id: 'font-save', label: 'Font preload (saved)', value: fontBoost, color: 'bg-green-500' });
      setWaterfall([...bars]);
    }
    ...
    setFcpValue(fcp);
    bars.push({ id: 'fcp', label: 'FCP', value: parseDuration, color: 'bg-yellow-500' });
    setWaterfall([...bars]);
    setPhase('done');
  };
    ...
    <section className="max-w-4xl mx-auto space-y-8 pb-16">
          ...
          {RESOURCES.map(r => (
            <div key={r.id} className="flex items-center gap-4">
              <ToggleSwitch
                ...
                activeColor={r.bad ? 'bg-red-600' : 'bg-green-600'}
                label={r.label}
              />
      ...
      {waterfall.length > 0 && (
        <div className="bg-gray-900 rounded-xl p-5 border border-gray-800 space-y-2">
          ...
          <WaterfallChart bars={waterfall} labelWidth="w-52" />
        </div>
      )}
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        ...
        <div className="p-6 min-h-[140px] flex items-center justify-center" aria-live="polite" aria-label="Mock page rendering state">
          ...
          {contentVisible && (
            <div className="w-full animate-fadeIn space-y-2">
              <div className="h-6 bg-gray-200 rounded w-1/2" aria-hidden="true" />
              <div className="h-4 bg-gray-300 rounded w-3/4" aria-hidden="true" />
